...
function renderRoll() {
  ...
  for (let i = 0; i < song.steps; i += 1) {
    headerCells.push(`<div class="tick">${(i % 4) + 1}</div>`);
  }
  ...
  const rowsHtml = PITCHES.map((pitch, rowIndex) => {
    ...
    for (let step = 0; step < song.steps; step += 1) {
      ...
      if (note?.start === step) {
        ...
        cells.push(`<div class="cell" data-row="${rowIndex}" data-step="${step}"><div class="note" data-id="${note.id}" style="width:${width}"></div></div>`);
      } else {
        cells.push(`<div class="cell" data-row="${rowIndex}" data-step="${step}"></div>`);
      }
    }
...
function updateSummary() {
  ...
  songSummary.textContent = `${song.title} | ${song.steps}스텝 | 노트 ${song.notes.length}개 | BPM ${song.tempo} | ${song.voice}`;
}
...
function createAudioEngine() {
  ...
  function play(song, handlers) {
    ...
    const secondsPerStep = 60 / song.tempo;
    currentStep = 0;
...
function playSynthTone(audioCtx, params) {
  const { freq, duration, voiceName } = params;
  ...
  amp.gain.linearRampToValueAtTime(preset.sustain, now + preset.attack);
  amp.gain.exponentialRampToValueAtTime(0.0006, now + duration * preset.tail);
  ...
  const oscillators = preset.partials.map((partial) => {
    ...
    partialGain.gain.exponentialRampToValueAtTime(0.0006, now + duration * preset.tail * partial.decay);

    osc.connect(partialGain);
    partialGain.connect(amp);
    osc.start(now);
    osc.stop(now + duration * preset.tail + 0.06);
    return osc;
  });
